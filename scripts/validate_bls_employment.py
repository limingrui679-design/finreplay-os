#!/usr/bin/env python3
"""Live-validate three archived BLS Employment Situation releases."""
...
from finreplay.adapters import AdapterBatch, BLSEmploymentSituationArchiveAdapter
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--raw-store",
        type=Path,
        default=Path("data/raw/supporting/bls-employment"),
    )
    parser.add_argument(
        "--receipt-directory",
        type=Path,
        default=Path("verification/supporting/bls-employment/live"),
    )
...
def main() -> None:
    ...
    for release_date in releases:
        # BLS archive delivery requires a contact-shaped user-agent token. The .invalid domain
        # identifies this reproducibility client without embedding a person's private address.
        with SafeHttpClient(
