#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the FHFA HPI boundary."""
...
REQUIRED_ASSERTIONS = (
    ...
    "post_event_lower_bound_breach_equals_60_basis_points",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_house_price_change_boundary_replaypack.py"
    verify_script = root / "scripts/verify_house_price_change_boundary_replaypack.py"

    lock = load_house_price_change_boundary_input_lock(input_path)
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match FHFA HPI input lock")
    ...
    range_artifact = f"{prefix}.shockcompiler.hpi-change-range"
    ...
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": "verification/supporting/fhfa-hpi/latest-summary.json",
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "absence_reasons": {
                    "observed": (
                        "The lock contains national monthly index changes reported in archived "
                        "FHFA releases, not property, borrower, mortgage, appraisal, transaction, "
                        "universal-home-price, causal, or future-event observations. The March "
                        "change is present only in the disjoint event lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    ...
                    "payload_pointer": (
                        "/naive_baseline/next_us_purchase_only_hpi_monthly_change_basis_points"
                    ),
                    "expected_value": 70,
                    "description": (
                        "The decision-time baseline persists the February first-report national "
                        "purchase-only seasonally adjusted HPI change of 70 basis points."
                    ),
                },
                {
                    "artifact_id": range_artifact,
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced April 2020 FHFA House Price Index "
                "boundary because two pre-decision first-report national monthly changes, two "
                "exact PDF hashes, one stable official-schedule semantic hash, exact 9:00 a.m. "
                "America/New_York timing, a separately locked May 26 March event, immutable locks, "
                "separated truth labels, an explicit February persistence baseline, a no-"
                ...
                "all verify. The schedule evidence does not claim that today's HTML bytes are an "
                "immutable 2019 snapshot, and the January report footer's '9AM EST' wording "
                "difference is retained rather than silently normalized. The March change is "
                "excluded from ReplayPack inputs and used only for a labelled post-event check "
                "that preserves its 60-basis-point lower-bound breach rather than widening the "
                "range. The May report's January and February snapshot values never overwrite the "
                "first-report inputs. The currently available May PDF has June 15 modification "
                "metadata, so its exact bytes are not represented as unchanged since May 26. "
                "FHFA's purchase-only repeat-transactions index is not every U.S. home, a property "
                "record, transaction count, appraisal, or mortgage outcome. This does not "
                "establish forecast skill, calibrated coverage, a contemporaneous COVID effect, "
                "housing, "
                "credit, pandemic, policy, regional, property, or firm causality, external "
