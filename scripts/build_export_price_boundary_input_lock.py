#!/usr/bin/env python3
"""Extract and seal the two BLS all-export releases known at decision time."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--replay-id",
        default="bls-export-prices-2020-all-exports-change-boundary-v1",
    )
    parser.add_argument("--artifact-prefix", default="bls-export-price")
    parser.add_argument(
        "--title",
        default="BLS all-export price monthly-change boundary before March 2020",
    )
    parser.add_argument(
        "--decision-time",
    ...
    parser.add_argument(
        "--build-epoch",
        type=_aware_datetime,
        default=_aware_datetime("2026-08-14T12:20:00Z"),
    )
    parser.add_argument(
        "--timevault",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("scenarios/bls-export-prices-2020/input-lock.json"),
    )
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"required BLS export-price facts are absent: {missing}")
    ...
    lock = ExportPriceBoundaryInputLock.create(
        {
            ...
            "roles": roles,
            "source_response_sha256s": BLS_EXPORT_PRICE_INPUT_RESPONSE_SHA256S,
            "supporting_receipt_sha256": (BLS_EXPORT_PRICE_SUPPORTING_RECEIPT_SHA256),
            "records": [record.model_dump(mode="json") for record in records],
            "claim_boundary": (
                "This lock preserves the January and February 2020 BLS all-export monthly "
                ...
                "excludes the March event, later revisions, exporter or shipment records, "
                "export quantities, nominal export values, tariffs, PPI, firm results, P&L, a "
    ...
    if args.output.exists() and args.output.read_bytes() != content:
        raise SystemExit("BLS export-price input-lock destination has different bytes")
