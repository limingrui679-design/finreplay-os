#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the March 2020 claims boundary."""
...
REQUIRED_ASSERTIONS = (
    ...
    "naive_baseline_is_latest_known_dol_claims_persistence",
    ...
    "post_event_upper_bound_breach_equals_2932000_persons",
    "reported_post_event_claims_breach_declared_range",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_initial_claims_boundary_replaypack.py"
    verify_script = root / "scripts/verify_initial_claims_boundary_replaypack.py"

    lock = load_initial_claims_boundary_input_lock(input_path)
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match DOL initial-claims input lock")
    ...
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": (
                "verification/supporting/dol-ui-claims/latest-summary.json"
            ),
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "inferred_artifact_ids": sorted(
                    (
                        f"{prefix}.shockcompiler.claims-range",
                        f"{prefix}.trialcourt.retrospective-gate",
                    )
                ),
                "bounded_artifact_ids": [f"{prefix}.shockcompiler.claims-range"],
                "simulated_artifact_ids": [f"{prefix}.trialcourt.retrospective-gate"],
                "absence_reasons": {
                    "observed": (
                        "The lock contains aggregate advance initial claims reported in archived "
                        "DOL releases, not individual claimants, employers, direct observations "
                        "of labor-market causes, policy effects, or future claims. The March 21 "
                        "value is only in the event lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    "artifact_id": f"{prefix}.shockcompiler.claims-range",
                    "payload_pointer": (
                        "/naive_baseline/"
                        "next_reported_seasonally_adjusted_initial_claims_persons"
                    ),
                    "expected_value": 281_000,
                    "description": (
                        "The decision-time baseline persists the latest known March 14 advance "
                        "seasonally adjusted initial-claims value of 281,000 persons."
                    ),
                },
                {
                    "artifact_id": f"{prefix}.shockcompiler.claims-range",
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced March 2020 DOL initial-claims "
                "boundary because two pre-decision archived PDF release snapshots, exact embargo "
                "timing plus official Last-Modified bounds, a separately locked post-decision "
                "March 21 claims value, an immutable input lock, separated truth labels, an "
                "explicit latest-value persistence baseline, a no-probability persistence-or-one-"
                "known-increase program, deliberate TrialCourt failure, deterministic outputs, "
                "limitations, and a clean-worktree double rebuild all verify. The March 21 value "
                "is excluded from ReplayPack inputs and used only for a labelled post-event check "
                "that preserves the 2,932,000-person range breach rather than widening the range. "
                "The later revision of March 14 from 281,000 to 282,000 stays in the event "
                "snapshot and never overwrites the decision input. This does not establish "
                "forecast skill, calibrated coverage, pandemic or labor-market causality, policy "
                "effectiveness, external validation, deployment, investment performance, or "
                "user impact."
            ),
        }
    )
