#!/usr/bin/env python3
"""Fresh-run relevant engines and byte-compare a BLS CPI boundary ReplayPack."""
...
def main() -> None:
    ...
    lock = load_cpi_boundary_input_lock(args.input_lock)
    ...
    with tempfile.TemporaryDirectory(prefix="finreplay-cpi-boundary-") as temporary:
        temporary_root = Path(temporary)
        first_spec = build_cpi_boundary_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
        first_root = studio.build(first_spec, temporary_root / "first").root
        second_spec = build_cpi_boundary_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
    ...
    if len(event_lock.records) != 1:
        raise SystemExit("CPI boundary event lock must contain exactly one record")
    event_record = event_lock.records[0]
    event_record_ids = {event_record.record_id}
    event_monthly = int(event_record.payload["value_tenths_percent"])
    baseline_monthly = int(
        shock["naive_baseline"]["next_release_monthly_change_tenths_percent"]
    )
    lower = int(shock["bound_construction"]["lower_monthly_change_tenths_percent"])
    upper = int(shock["bound_construction"]["upper_monthly_change_tenths_percent"])
    assertions = {
        ...
        "naive_baseline_is_latest_known_cpi_persistence": baseline_monthly == 5,
        ...
        "post_decision_event_is_exact_archived_cpi_fact": (
            event_record.source.source_id == BLS_CPI_SOURCE_ID
            and event_record.payload.get("release_date") == "2023-03-14"
            and event_record.payload.get("report_period") == "2023-02"
        ...
        "reported_post_event_monthly_inside_declared_range": lower
        <= event_monthly
        <= upper,
    ...
    if not all(assertions.values()):
        ...
        raise SystemExit(f"CPI boundary assertions failed: {failed}")
    semantic: dict[str, Any] = {
        ...
        "claim_boundary": (
            ...
            "BLS headline facts. It verifies the separately locked March 14 CPI monthly-change "
            "fact is disjoint and, as an evaluation only, falls inside the previously declared "
            "no-probability range. It does not prove forecast skill, calibrated coverage, a "
            "stationary sample across annual weight updates and seasonal recalculation, causal "
            "inflation attribution, policy effectiveness, deployment, external review, or user "
            "impact."
        ),
        ...
        "post_event_evaluation": {
            "reported_february_monthly_change_tenths_percent": event_monthly,
            "latest_known_persistence_baseline_tenths_percent": baseline_monthly,
            "declared_lower_monthly_change_tenths_percent": lower,
            "declared_upper_monthly_change_tenths_percent": upper,
    ...
    if args.write_receipt:
        ...
        raise SystemExit("CPI boundary receipt is missing; use --write-receipt once")
    print(
...
def _verify_stored_receipt(
    ...
    if receipt_sha256 != _hash(stored):
        raise SystemExit("stored CPI boundary receipt_sha256 mismatch")
    stored.pop("runtime", None)
    semantic_sha256 = stored.pop("semantic_sha256", None)
    if semantic_sha256 != _hash(stored):
        raise SystemExit("stored CPI boundary semantic_sha256 mismatch")
    if semantic_sha256 != expected_semantic_sha256 or stored != expected_semantic:
        raise SystemExit("stored CPI boundary semantic receipt differs from fresh rebuild")
