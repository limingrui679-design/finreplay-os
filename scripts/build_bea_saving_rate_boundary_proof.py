#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the BEA saving-rate boundary."""
...
REQUIRED_ASSERTIONS = (
    ...
    "naive_baseline_is_latest_known_saving_rate_persistence",
    ...
    "post_event_upper_bound_breach_equals_460_basis_points",
    "reported_post_event_rate_is_above_declared_range",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_bea_saving_rate_boundary_replaypack.py"
    verify_script = root / "scripts/verify_bea_saving_rate_boundary_replaypack.py"

    lock = load_bea_saving_rate_boundary_input_lock(input_path)
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match BEA saving-rate input lock")
    ...
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": "verification/supporting/bea-pio/latest-summary.json",
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "inferred_artifact_ids": sorted(
                    (
                        f"{prefix}.shockcompiler.saving-rate-range",
                        f"{prefix}.trialcourt.retrospective-gate",
                    )
                ),
                "bounded_artifact_ids": [f"{prefix}.shockcompiler.saving-rate-range"],
                "simulated_artifact_ids": [f"{prefix}.trialcourt.retrospective-gate"],
                "absence_reasons": {
                    "observed": (
                        "The lock contains aggregate personal-saving-rate estimates reported "
                        "in archived BEA releases, not household-level observations, direct "
                        "behavioral measures, causal effects, transactions, positions, or a "
                        "future saving rate. The March value is only in the event lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    "artifact_id": f"{prefix}.shockcompiler.saving-rate-range",
                    "payload_pointer": ("/naive_baseline/next_personal_saving_rate_basis_points"),
                    "expected_value": 820,
                    "description": (
                        "The decision-time baseline persists the latest known February personal "
                        "saving rate of 820 basis points from its March 27 release snapshot."
                    ),
                },
                {
                    "artifact_id": f"{prefix}.shockcompiler.saving-rate-range",
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced March 2020 BEA personal-saving-rate "
                "boundary because two pre-decision archived HTML/PDF release snapshots, exact "
                "8:30 a.m. America/New_York embargo timing, a separately locked post-decision "
                "March saving-rate release, immutable locks, separated truth labels, an explicit "
                "latest-value persistence baseline, a no-probability persistence-or-one-known-"
                "increase program, deliberate TrialCourt failure, deterministic outputs, "
                "limitations, and a clean-worktree double rebuild all verify. The March value is "
                "excluded from ReplayPack inputs and used only for a labelled post-event check "
                "that preserves its 460-basis-point upper-bound breach rather than widening the "
                "range. The April release's revision of February from 8.2 to 8.0 percent stays "
                "in the event snapshot and never overwrites the decision input. This does not "
                "establish forecast skill, calibrated coverage, household behavior, pandemic or "
                "policy causality, external validation, deployment, investment performance, or "
                "user impact."
            ),
        }
    )
