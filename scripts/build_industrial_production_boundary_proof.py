#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the Federal Reserve G.17 boundary."""
...
REQUIRED_ASSERTIONS = (
    ...
    "post_event_lower_bound_breach_equals_600_basis_points",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_industrial_production_boundary_replaypack.py"
    verify_script = root / "scripts/verify_industrial_production_boundary_replaypack.py"

    lock = load_industrial_production_boundary_input_lock(input_path)
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match G.17 input lock")
    ...
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": "verification/supporting/fed-g17/latest-summary.json",
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "absence_reasons": {
                    "observed": (
                        "The lock contains aggregate industrial-production changes reported in "
                        "archived Federal Reserve releases, not plant-level observations, direct "
                        "causal measurements, transactions, positions, or a future production "
                        "change. The March value is only in the event lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    ...
                    "description": (
                        ...
                        "industrial-production monthly change of 60 basis points from its March "
                        "17 archived release snapshot."
                    ),
                },
                {
                    "artifact_id": f"{prefix}.shockcompiler.monthly-change-range",
                    ...
                    "description": (
                        "The persistence-or-one-known-increase endpoints carry no probability or "
                        "coverage claim."
                    ),
                },
            ],
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced March 2020 Federal Reserve G.17 "
                "industrial-production boundary because two pre-decision archived HTML/PDF "
                "release snapshots, exact 9:15 a.m. America/New_York publication timing, a "
                "separately locked post-decision March release, immutable locks, separated truth "
                "labels, an explicit latest-value persistence baseline, a no-probability "
                ...
                "labelled post-event check that preserves its 600-basis-point lower-bound breach "
                "rather than widening the range. The April release's revision of February from "
                "0.6 to 0.5 percent stays in the event snapshot and never overwrites the decision "
                "input. This does not establish forecast skill, calibrated coverage, plant, "
                "pandemic, policy, sector, or industrial causality, external validation, "
