#!/usr/bin/env python3
"""Fresh-run relevant engines and byte-compare the September 2019 SOFR ReplayPack."""
...
def main() -> None:
    ...
    lock = load_sofr_boundary_input_lock(args.input_lock)
    ...
    if event_lock.scenario_id != lock.scenario_id:
        raise SystemExit("event-lock scenario_id does not match SOFR input lock")
    if event_lock.scenario_version != lock.scenario_version:
        raise SystemExit("event-lock scenario_version does not match SOFR input lock")
    if event_lock.decision_time != lock.decision_time:
        raise SystemExit("event-lock decision_time does not match SOFR input lock")

    started = time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="finreplay-sofr-boundary-") as temporary:
        temporary_root = Path(temporary)
        first_spec = build_sofr_boundary_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
        first_root = studio.build(first_spec, temporary_root / "first").root
        second_spec = build_sofr_boundary_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
    ...
    if len(event_lock.records) != 1:
        raise SystemExit("SOFR boundary event lock must contain exactly one record")
    ...
    assertions = {
        ...
        "naive_baseline_is_latest_known_sofr_persistence": baseline_rate == 243,
        ...
        "post_decision_event_is_exact_final_nyfed_sofr_fact": (
            event_record.source.source_id == NYFED_SOFR_SOURCE_ID
            and event_record.entity_id == "nyfed_reference_rate:SOFR"
            and event_record.payload.get("effective_date") == "2019-09-17"
            and event_record.payload.get("publication_business_date") == "2019-09-18"
            and event_record.payload.get("rate_type") == "SOFR"
            and event_record.payload.get("unit") == "Basis Points"
            and event_record.payload.get("revision_indicator") == ""
            and event_record.payload.get("ancillary_statistics_normalized") is False
            and event_record.payload.get("reported_value_percent") == "5.25"
            and Decimal(str(event_record.payload.get("reported_value_percent"))) * 100
            == event_rate
            and event_rate == 525
        ),
        ...
        "reported_post_event_rate_breaches_declared_range": event_rate > upper,
        "post_event_upper_bound_breach_equals_282_basis_points": breach == 282,
    ...
    if not all(assertions.values()):
        ...
        raise SystemExit(f"SOFR boundary assertions failed: {failed}")
    semantic: dict[str, Any] = {
        ...
        "claim_boundary": (
            ...
            "final New York Fed SOFR facts. It verifies the separately locked September 17 final "
            "SOFR rate is disjoint and, as an evaluation only, breaches the previously declared "
            "no-probability range by 282 basis points. The range is not widened after the fact. "
            "This does not prove forecast skill, calibrated coverage, repo-market causality, "
            "policy effectiveness, deployment, external review, or user impact."
        ),
        "assertions": assertions,
        "input_lock_sha256": lock.lock_sha256,
        "event_lock_sha256": event_lock.lock_sha256,
        "post_event_evaluation": {
            "reported_september17_final_sofr_basis_points": event_rate,
            ...
            "upper_bound_breach_basis_points": breach,
            "range_breached": True,
    ...
    if args.write_receipt:
        ...
        raise SystemExit("SOFR boundary receipt is missing; use --write-receipt once")
    print(
...
def _verify_stored_receipt(
    ...
    if receipt_sha256 != _hash(stored):
        raise SystemExit("stored SOFR boundary receipt_sha256 mismatch")
    stored.pop("runtime", None)
    semantic_sha256 = stored.pop("semantic_sha256", None)
    if semantic_sha256 != _hash(stored):
        raise SystemExit("stored SOFR boundary semantic_sha256 mismatch")
    if semantic_sha256 != expected_semantic_sha256 or stored != expected_semantic:
        raise SystemExit("stored SOFR boundary semantic receipt differs from fresh rebuild")
