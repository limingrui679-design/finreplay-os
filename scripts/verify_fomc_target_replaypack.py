#!/usr/bin/env python3
"""Fresh-run relevant engines and byte-compare a FOMC target-range boundary ReplayPack."""
...
def main() -> None:
    ...
    lock = load_fomc_target_boundary_input_lock(args.input_lock)
    ...
    with tempfile.TemporaryDirectory(prefix="finreplay-fomc-target-boundary-") as temporary:
        temporary_root = Path(temporary)
        first_spec = build_fomc_target_boundary_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
        first_root = studio.build(first_spec, temporary_root / "first").root
        second_spec = build_fomc_target_boundary_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
    ...
    event_upper = int(event_lock.records[0].payload["value_basis_points"])
    baseline_upper = int(
        shock["naive_baseline"]["next_target_range_upper_basis_points"]
    )
    lower = int(shock["bound_construction"]["lower_next_upper_basis_points"])
    upper = int(shock["bound_construction"]["upper_next_upper_basis_points"])
    assertions = {
        ...
        "naive_baseline_is_latest_known_upper_target_persistence": baseline_upper == 500,
        ...
        "reported_post_event_upper_target_inside_declared_range": lower
        <= event_upper
        <= upper,
    ...
    if not all(assertions.values()):
        ...
        raise SystemExit(f"FOMC target boundary assertions failed: {failed}")
    semantic: dict[str, Any] = {
        ...
        "claim_boundary": (
            ...
            "FOMC target facts. It verifies the separately locked May 3 upper target is disjoint "
            "and, as an evaluation only, falls inside the previously declared no-probability "
            "range. It does not prove forecast skill, calibrated coverage, monetary-policy "
            "correctness or causal effects, policy effectiveness, deployment, external review, "
            "or user impact."
        ),
        ...
        "post_event_evaluation": {
            "reported_may_upper_target_basis_points": event_upper,
            "latest_known_persistence_baseline_basis_points": baseline_upper,
            "declared_lower_upper_target_basis_points": lower,
            "declared_upper_upper_target_basis_points": upper,
    ...
    if args.write_receipt:
        ...
        raise SystemExit("FOMC target boundary receipt is missing; use --write-receipt once")
    print(
...
def _verify_stored_receipt(
    ...
    if receipt_sha256 != _hash(stored):
        raise SystemExit("stored FOMC target boundary receipt_sha256 mismatch")
    stored.pop("runtime", None)
    semantic_sha256 = stored.pop("semantic_sha256", None)
    if semantic_sha256 != _hash(stored):
        raise SystemExit("stored FOMC target boundary semantic_sha256 mismatch")
    if semantic_sha256 != expected_semantic_sha256 or stored != expected_semantic:
        raise SystemExit("stored FOMC target boundary semantic receipt differs from fresh rebuild")
