#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the BLS payroll boundary ReplayPack."""
...
REQUIRED_ASSERTIONS = (
    ...
    "naive_baseline_is_latest_known_payroll_persistence",
    ...
    "reported_post_event_payroll_inside_declared_range",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_employment_boundary_replaypack.py"
    verify_script = root / "scripts/verify_employment_boundary_replaypack.py"

    lock = load_employment_boundary_input_lock(input_path)
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match employment boundary input lock")
    ...
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": (
                "verification/supporting/bls-employment/latest-summary.json"
            ),
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "inferred_artifact_ids": sorted(
                    (
                        f"{prefix}.shockcompiler.payroll-range",
                        f"{prefix}.trialcourt.retrospective-gate",
                    )
                ),
                "bounded_artifact_ids": [f"{prefix}.shockcompiler.payroll-range"],
                "simulated_artifact_ids": [f"{prefix}.trialcourt.retrospective-gate"],
                "absence_reasons": {
                    "observed": (
                        "The lock contains aggregate headline facts reported in BLS releases, not "
                        "direct worker, employer, payroll-record, or causal labor observations. "
                        "The later headline fact is only in the event lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    "artifact_id": f"{prefix}.shockcompiler.payroll-range",
                    "payload_pointer": (
                        "/naive_baseline/next_headline_payroll_change_thousands"
                    ),
                    "expected_value": 517,
                    "description": (
                        "The explicit decision-time baseline persists the latest known January "
                        "headline payroll change of 517 thousand."
                    ),
                },
                {
                    "artifact_id": f"{prefix}.shockcompiler.payroll-range",
                    ...
                    "description": (
                        "The two payroll endpoints carry no probability or coverage claim."
                    ),
                },
            ],
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced BLS payroll-release boundary because "
                "four official pre-decision headline facts, exact embargo-end knowledge timing, a "
                ...
                "bounded payroll program, deliberate TrialCourt failure, deterministic outputs, "
                "limitations, and a clean-worktree double rebuild all verify. The later headline "
                "is excluded from ReplayPack inputs and used only for a labelled post-event range "
                "check. This does not establish forecast skill, calibrated coverage, stationarity "
                "across annual benchmarking, causal labor-market attribution, policy "
                "effectiveness, external validation, deployment, investment performance, or user "
                "impact."
            ),
        }
    )
