#!/usr/bin/env python3
"""Fresh-run every engine and byte-compare a committed bank boundary ReplayPack."""
...
def main() -> None:
    ...
    lock = load_bank_boundary_input_lock(args.input_lock)
    ...
    with tempfile.TemporaryDirectory(prefix="finreplay-bank-boundary-") as temporary:
        temporary_root = Path(temporary)
        first_spec = build_bank_boundary_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
        first_root = studio.build(first_spec, temporary_root / "first").root
        second_spec = build_bank_boundary_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
    ...
    if not all(assertions.values()):
        ...
        raise SystemExit(f"bank seven-engine assertions failed: {failed}")
    semantic: dict[str, Any] = {
        "schema_version": "1.0.0",
        "claim_boundary": (
            "This receipt proves two fresh local executions of every engine rebuild the "
            "committed directory and deterministic ZIP bytes from the locked SEC facts, while "
            "the separately locked post-decision official event remains excluded. It does not "
            "prove source authenticity beyond the locks, causal correctness, deployment, "
            "trading performance, external review, or user impact."
        ),
    ...
    if args.write_receipt:
        ...
        raise SystemExit("bank run receipt is missing; use --write-receipt once")
    print(
...
def _verify_stored_receipt(
    ...
    if receipt_sha256 != _hash(stored):
        raise SystemExit("stored bank run receipt_sha256 mismatch")
    stored.pop("runtime", None)
    semantic_sha256 = stored.pop("semantic_sha256", None)
    if semantic_sha256 != _hash(stored):
        raise SystemExit("stored bank semantic_sha256 mismatch")
    if semantic_sha256 != expected_semantic_sha256 or stored != expected_semantic:
        raise SystemExit("stored bank semantic receipt differs from fresh rebuild")
