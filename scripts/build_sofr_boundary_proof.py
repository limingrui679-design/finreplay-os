#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the September 2019 SOFR boundary."""
...
REQUIRED_ASSERTIONS = (
    ...
    "naive_baseline_is_latest_known_sofr_persistence",
    ...
    "post_event_upper_bound_breach_equals_282_basis_points",
    "reported_post_event_rate_breaches_declared_range",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_sofr_boundary_replaypack.py"
    verify_script = root / "scripts/verify_sofr_boundary_replaypack.py"

    lock = load_sofr_boundary_input_lock(input_path)
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match SOFR boundary input lock")
    ...
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": (
                "verification/supporting/nyfed-sofr-history/latest-summary.json"
            ),
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "inferred_artifact_ids": sorted(
                    (
                        f"{prefix}.shockcompiler.rate-range",
                        f"{prefix}.trialcourt.retrospective-gate",
                    )
                ),
                "bounded_artifact_ids": [f"{prefix}.shockcompiler.rate-range"],
                "simulated_artifact_ids": [f"{prefix}.trialcourt.retrospective-gate"],
                "absence_reasons": {
                    "observed": (
                        "The lock contains aggregate final SOFR rates reported by the New York "
                        "Fed, not transaction-level repo observations, causal market effects, or "
                        "future rates. The September 17 rate is only in the event lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    "artifact_id": f"{prefix}.shockcompiler.rate-range",
                    "payload_pointer": "/naive_baseline/next_final_sofr_basis_points",
                    "expected_value": 243,
                    "description": (
                        "The decision-time baseline persists the latest known September 16 final "
                        "SOFR rate of 243 basis points."
                    ),
                },
                {
                    "artifact_id": f"{prefix}.shockcompiler.rate-range",
                    ...
                    "description": (
                        "The two final-SOFR endpoints carry no probability or coverage claim."
                    ),
                },
            ],
            "deliberate_failure_modes": [
                {
                    ...
                    "description": (
                        "TrialCourt must reject the retrospective two-rate attempt rather than "
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced September 2019 SOFR boundary because "
                "two pre-decision final New York Fed rates, conservative post-revision-window "
                "knowledge timing, a separately locked post-decision September 17 rate, an "
                "immutable input lock, separated truth labels, an explicit latest-rate "
                ...
                "all verify. The September 17 rate is excluded from ReplayPack inputs and used "
                "only for a labelled post-event check that preserves the 282-basis-point range "
                "breach rather than widening the range. This does not establish forecast skill, "
                "calibrated coverage, repo-market causality, policy effectiveness, external "
