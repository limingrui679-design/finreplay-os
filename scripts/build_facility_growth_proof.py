#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the BTFP growth ReplayPack."""
...
REQUIRED_ASSERTIONS = (
    ...
    "reported_post_event_growth_inside_declared_envelope",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_facility_growth_replaypack.py"
    verify_script = root / "scripts/verify_facility_growth_replaypack.py"

    lock = load_facility_growth_input_lock(input_path)
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match facility growth input lock")
    ...
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": "verification/supporting/fed-h41/latest-summary.json",
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "inferred_artifact_ids": sorted(
                    (
                        f"{prefix}.shockcompiler.growth-bound",
                        f"{prefix}.trialcourt.retrospective-gate",
                    )
                ),
                "bounded_artifact_ids": [f"{prefix}.shockcompiler.growth-bound"],
                "simulated_artifact_ids": [f"{prefix}.trialcourt.retrospective-gate"],
                "absence_reasons": {
                    "observed": (
                        "The lock contains aggregate balances reported in Federal Reserve H.4.1 "
                        "releases, not direct borrower, collateral, transaction, or stress "
                        "observations. The later balance is only in the event lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    "artifact_id": f"{prefix}.shockcompiler.growth-bound",
                    "payload_pointer": "/naive_baseline/next_week_growth_millions",
                    "expected_value": 0.0,
                    "description": (
                        "The explicit decision-time baseline assumes no additional next-week BTFP "
                        "Wednesday balance growth."
                    ),
                },
                {
                    "artifact_id": f"{prefix}.shockcompiler.growth-bound",
                    ...
                    "description": (
                        "The growth endpoints deliberately carry no probability or coverage claim."
                    ),
                },
            ],
            "deliberate_failure_modes": [
                {
                    ...
                    "description": (
                        "TrialCourt must reject the retrospective one-week attempt rather than "
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced Federal Reserve facility-growth "
                "boundary because four official pre-decision H.4.1 facts, deterministic "
                "conservative knowledge timing, a separately locked post-decision release fact, "
                "an immutable input lock, separated truth labels, an explicit zero-growth naive "
                "baseline, a no-probability bounded growth program, deliberate TrialCourt failure, "
                "deterministic outputs, limitations, and a clean-worktree double rebuild all "
                "verify. The later balance is excluded from ReplayPack inputs and used only for a "
                "labelled post-event coverage check. This does not establish forecasting skill, "
                "calibrated coverage, causal systemic stress, policy effectiveness, external "
