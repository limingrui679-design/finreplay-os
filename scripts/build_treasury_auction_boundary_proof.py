#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the Treasury auction boundary."""
...
REQUIRED_ASSERTIONS = (
    ...
    "naive_baseline_is_latest_known_high_rate_persistence",
    ...
    "post_event_lower_bound_breach_equals_19_basis_points",
    "reported_post_event_rate_is_below_declared_range",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_treasury_auction_boundary_replaypack.py"
    verify_script = root / "scripts/verify_treasury_auction_boundary_replaypack.py"

    lock = load_treasury_auction_boundary_input_lock(input_path)
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match Treasury auction input lock")
    ...
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": (
                "verification/supporting/treasury-auction-results/latest-summary.json"
            ),
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "inferred_artifact_ids": sorted(
                    (
                        f"{prefix}.shockcompiler.high-rate-range",
                        f"{prefix}.trialcourt.retrospective-gate",
                    )
                ),
                "bounded_artifact_ids": [f"{prefix}.shockcompiler.high-rate-range"],
                "simulated_artifact_ids": [f"{prefix}.trialcourt.retrospective-gate"],
                "absence_reasons": {
                    "observed": (
                        "The lock contains aggregate Treasury auction results reported in "
                        "paired official XML/PDF releases, not individual bids, bidder "
                        "motivations, orders, policy effects, or future auction rates. The "
                        "March 23 zero-rate result is only in the event lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    "artifact_id": f"{prefix}.shockcompiler.high-rate-range",
                    "payload_pointer": ("/naive_baseline/next_91_day_bill_high_rate_basis_points"),
                    "expected_value": 29,
                    "description": (
                        "The decision-time baseline persists the latest known March 16 "
                        "91-day bill high discount rate of 29 basis points."
                    ),
                },
                {
                    "artifact_id": f"{prefix}.shockcompiler.high-rate-range",
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced March 2020 Treasury 91-day bill "
                "auction-rate boundary because two pre-decision paired official XML/PDF result "
                "snapshots, explicit XML release times interpreted under Treasury's stated "
                "timeline and conservatively delayed to next local midnight, a separately "
                "locked post-decision zero-rate result, immutable locks, separated truth labels, "
                "an explicit latest-rate persistence baseline, a no-probability persistence-or-"
                "one-known-decline program, deliberate TrialCourt failure, deterministic "
                "outputs, limitations, and a clean-worktree double rebuild all verify. The "
                "March 23 value is excluded from ReplayPack inputs and used only for a labelled "
                "post-event check that preserves its 19-basis-point lower-bound breach rather "
                "than widening the range. This does not establish forecast skill, calibrated "
                "coverage, bidder demand or policy causality, investment performance, external "
