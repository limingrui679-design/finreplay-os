#!/usr/bin/env python3
"""Fresh-run relevant engines and byte-compare a BLS payroll boundary ReplayPack."""
...
def main() -> None:
    ...
    lock = load_employment_boundary_input_lock(args.input_lock)
    ...
    with tempfile.TemporaryDirectory(prefix="finreplay-employment-boundary-") as temporary:
        temporary_root = Path(temporary)
        first_spec = build_employment_boundary_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
        first_root = studio.build(first_spec, temporary_root / "first").root
        second_spec = build_employment_boundary_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
    ...
    event_payroll = int(event_lock.records[0].payload["value_thousands"])
    baseline_payroll = int(
        shock["naive_baseline"]["next_headline_payroll_change_thousands"]
    )
    lower = int(shock["bound_construction"]["lower_payroll_change_thousands"])
    upper = int(shock["bound_construction"]["upper_payroll_change_thousands"])
    assertions = {
        ...
        "reported_post_event_payroll_inside_declared_range": lower
        <= event_payroll
        <= upper,
    ...
    if not all(assertions.values()):
        ...
        raise SystemExit(f"employment boundary assertions failed: {failed}")
    semantic: dict[str, Any] = {
        ...
        "claim_boundary": (
            ...
            "BLS headline facts. It verifies the separately locked March 10 payroll fact is "
            "disjoint and, as an evaluation only, falls inside the previously declared no-"
            "probability range. It does not prove forecast skill, calibrated coverage, a "
            "stationary sample across annual benchmarking, causal labor-market attribution, "
        ...
        "post_event_evaluation": {
            "reported_february_payroll_change_thousands": event_payroll,
            "latest_known_persistence_baseline_thousands": baseline_payroll,
            "declared_lower_payroll_change_thousands": lower,
            "declared_upper_payroll_change_thousands": upper,
    ...
    if args.write_receipt:
        ...
        raise SystemExit("employment boundary receipt is missing; use --write-receipt once")
    print(
...
def _verify_stored_receipt(
    ...
    if receipt_sha256 != _hash(stored):
        raise SystemExit("stored employment boundary receipt_sha256 mismatch")
    stored.pop("runtime", None)
    semantic_sha256 = stored.pop("semantic_sha256", None)
    if semantic_sha256 != _hash(stored):
        raise SystemExit("stored employment boundary semantic_sha256 mismatch")
    if semantic_sha256 != expected_semantic_sha256 or stored != expected_semantic:
        raise SystemExit("stored employment boundary semantic receipt differs from fresh rebuild")
