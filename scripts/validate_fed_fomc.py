#!/usr/bin/env python3
"""Live-validate three archived FOMC statements used by one scenario."""
...
from finreplay.adapters import AdapterBatch, FederalReserveFOMCStatementAdapter
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="ISO date; repeat to override the three scenario statements.",
    )
    parser.add_argument(
        "--database",
        type=Path,
    ...
    parser.add_argument(
        "--raw-store",
        type=Path,
        default=Path("data/raw/supporting/fed-fomc"),
    )
    parser.add_argument(
        "--receipt-directory",
        type=Path,
        default=Path("verification/supporting/fed-fomc/live"),
    )
