#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the FOMC target-range boundary ReplayPack."""
...
REQUIRED_ASSERTIONS = (
    ...
    "naive_baseline_is_latest_known_upper_target_persistence",
    ...
    "reported_post_event_upper_target_inside_declared_range",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_fomc_target_replaypack.py"
    verify_script = root / "scripts/verify_fomc_target_replaypack.py"

    lock = load_fomc_target_boundary_input_lock(input_path)
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match FOMC target boundary input lock")
    ...
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": (
                "verification/supporting/fed-fomc/latest-summary.json"
            ),
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "inferred_artifact_ids": sorted(
                    (
                        f"{prefix}.shockcompiler.upper-bound",
                        f"{prefix}.trialcourt.retrospective-gate",
                    )
                ),
                "bounded_artifact_ids": [f"{prefix}.shockcompiler.upper-bound"],
                "simulated_artifact_ids": [f"{prefix}.trialcourt.retrospective-gate"],
                "absence_reasons": {
                    "observed": (
                        "The lock contains target endpoints reported in FOMC statements, not "
                        "direct market observations, expectations, votes, or causal policy "
                        "effects. The later upper target is only in the event lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    "artifact_id": f"{prefix}.shockcompiler.upper-bound",
                    "payload_pointer": (
                        "/naive_baseline/next_target_range_upper_basis_points"
                    ),
                    "expected_value": 500,
                    "description": (
                        "The explicit decision-time baseline persists the latest known 500-basis-"
                        "point upper target endpoint."
                    ),
                },
                {
                    "artifact_id": f"{prefix}.shockcompiler.upper-bound",
                    ...
                    "description": (
                        "The two next-upper endpoints carry no probability or coverage claim."
                    ),
                },
            ],
            "deliberate_failure_modes": [
                {
                    ...
                    "description": (
                        "TrialCourt must reject the retrospective policy-step attempt rather than "
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced FOMC target-range boundary because "
                "four official pre-decision target endpoints, exact page-stated release timing, a "
                "separately locked post-decision upper target, an immutable input lock, separated "
                "truth labels, an explicit latest-upper persistence baseline, a no-probability "
                "zero-or-one-known-step program, deliberate TrialCourt failure, deterministic "
                "outputs, limitations, and a clean-worktree double rebuild all verify. The later "
                "upper target is excluded from ReplayPack inputs and used only for a labelled "
                "post-event range check. This does not establish forecast skill, calibrated "
                "coverage, monetary-policy correctness or causal effects, external validation, "
