#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the Federal Reserve G.19 boundary."""
...
REQUIRED_ASSERTIONS = (
    ...
    "naive_baseline_is_latest_known_value_persistence",
    ...
    "post_event_lower_bound_breach_equals_3550_basis_points",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_consumer_credit_boundary_replaypack.py"
    verify_script = root / "scripts/verify_consumer_credit_boundary_replaypack.py"

    lock = load_consumer_credit_boundary_input_lock(input_path)
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match G.19 input lock")
    ...
    shock_id = f"{prefix}.shockcompiler.revolving-credit-change-range"
    trial_id = f"{prefix}.trialcourt.retrospective-gate"
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": "verification/supporting/fed-g19/latest-summary.json",
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "inferred_artifact_ids": sorted((shock_id, trial_id)),
                "bounded_artifact_ids": [shock_id],
                "simulated_artifact_ids": [trial_id],
                "absence_reasons": {
                    "observed": (
                        "The lock contains aggregate revolving-credit changes reported in one "
                        "archived Federal Reserve G.19 release, not household, account, card, "
                        "transaction, causal, or future-event observations. The March value is "
                        "present only in the disjoint event lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    ...
                    "payload_pointer": (
                        "/naive_baseline/"
                        "next_revolving_credit_change_annual_rate_basis_points"
                    ),
                    "expected_value": 460,
                    "description": (
                        "The decision-time baseline persists the latest known February G.19 "
                        "revolving-credit table value of 460 basis points from the April 7 "
                        "archived release snapshot."
                    ),
                },
                {
                    "artifact_id": shock_id,
                ...
                },
                {
                    "artifact_id": shock_id,
                    "payload_pointer": (
                        "/bound_construction/table_values_not_rounded_headline_fractions"
                    ),
                    "expected_value": True,
                    "description": (
                        "The boundary retains the official table's one-decimal values because "
                        "G.19 says its simple annual rates are calculated from unrounded data."
                    ),
                },
            ],
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced March 2020 Federal Reserve G.19 "
                "revolving-credit boundary because two pre-decision reported monthly values in "
                "one archived April 7 PDF, exact 3 p.m. America/New_York publication timing, a "
                "separately locked May 7 post-decision March value, immutable locks, separated "
                "truth labels, an explicit latest-value persistence baseline, a no-probability "
                ...
                "labelled post-event check that preserves its 3,550-basis-point lower-bound "
                "breach rather than widening the range. The May release's 100-basis-point "
                "downward revisions to January and February stay in the event snapshot and "
                "never overwrite the April decision inputs. The table values are simple annual "
                "rates calculated from unrounded data; they are not card-spending or transaction "
                "measures. This does not establish forecast skill, calibrated coverage, "
                "household, consumer, pandemic, policy, lender, or macroeconomic causality, "
