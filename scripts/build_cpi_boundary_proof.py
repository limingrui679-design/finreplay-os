#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the BLS CPI monthly-change boundary ReplayPack."""
...
REQUIRED_ASSERTIONS = (
    ...
    "naive_baseline_is_latest_known_cpi_persistence",
    ...
    "reported_post_event_monthly_inside_declared_range",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_cpi_boundary_replaypack.py"
    verify_script = root / "scripts/verify_cpi_boundary_replaypack.py"

    lock = load_cpi_boundary_input_lock(input_path)
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match CPI boundary input lock")
    ...
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": (
                "verification/supporting/bls-cpi/latest-summary.json"
            ),
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "inferred_artifact_ids": sorted(
                    (
                        f"{prefix}.shockcompiler.cpi-range",
                        f"{prefix}.trialcourt.retrospective-gate",
                    )
                ),
                "bounded_artifact_ids": [f"{prefix}.shockcompiler.cpi-range"],
                "simulated_artifact_ids": [f"{prefix}.trialcourt.retrospective-gate"],
                "absence_reasons": {
                    "observed": (
                        "The lock contains aggregate headline facts reported in BLS releases, "
                        "not direct price quotes, item-level records, or causal inflation "
                        "observations. "
                        "The March monthly-change headline fact is only in the event lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    "artifact_id": f"{prefix}.shockcompiler.cpi-range",
                    "payload_pointer": (
                        "/naive_baseline/next_release_monthly_change_tenths_percent"
                    ),
                    "expected_value": 5,
                    "description": (
                        "The explicit decision-time baseline persists the latest known January "
                        "headline monthly change of 0.5 percent."
                    ),
                },
                {
                    "artifact_id": f"{prefix}.shockcompiler.cpi-range",
                    ...
                    "description": (
                        "The two CPI monthly-change endpoints carry no probability or coverage "
                        "claim."
                    ),
                },
            ],
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced BLS CPI monthly-change release "
                "boundary because four official pre-decision headline facts, exact embargo-end "
                "knowledge timing, a "
                "separately locked post-decision headline fact, an immutable input lock, separated "
                "truth labels, an explicit latest-value persistence baseline, a no-probability "
                "bounded CPI monthly-change program, deliberate TrialCourt failure, deterministic "
                "outputs, limitations, and a clean-worktree double rebuild all verify. The March "
                "monthly-change headline is excluded from ReplayPack inputs and used only for a "
                "labelled post-event range check. This does not establish forecast skill, "
                "calibrated coverage, stationarity across annual weight updates and seasonal "
                "recalculation, causal inflation attribution, policy effectiveness, external "
