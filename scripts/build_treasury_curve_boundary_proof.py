#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the Treasury-curve boundary ReplayPack."""
...
REQUIRED_ASSERTIONS = (
    ...
    "naive_baseline_is_latest_known_spread_persistence",
    ...
    "post_event_range_breach_equals_six_basis_points",
    "post_event_spread_breaches_declared_range",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_treasury_curve_boundary_replaypack.py"
    verify_script = root / "scripts/verify_treasury_curve_boundary_replaypack.py"

    lock = load_treasury_curve_boundary_input_lock(input_path)
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match Treasury-curve boundary input lock")
    ...
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": (
                "verification/supporting/alfred-treasury-yields/latest-summary.json"
            ),
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "inferred_artifact_ids": sorted(
                    (
                        f"{prefix}.shockcompiler.spread-range",
                        f"{prefix}.trialcourt.retrospective-gate",
                    )
                ),
                "bounded_artifact_ids": [f"{prefix}.shockcompiler.spread-range"],
                "simulated_artifact_ids": [f"{prefix}.trialcourt.retrospective-gate"],
                "absence_reasons": {
                    "observed": (
                        "The lock contains constant-maturity yields reported in native ALFRED "
                        "vintages, not direct security trades, positions, flows, or causal "
                        "yield-curve observations. The March 15 yield pair is only in the event "
                        "lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    "artifact_id": f"{prefix}.shockcompiler.spread-range",
                    "payload_pointer": (
                        "/naive_baseline/next_dgs10_minus_dgs2_spread_basis_points"
                    ),
                    "expected_value": -48,
                    "description": (
                        "The decision-time baseline persists the latest known March 13 derived "
                        "DGS10-minus-DGS2 spread of -48 basis points."
                    ),
                },
                {
                    "artifact_id": f"{prefix}.shockcompiler.spread-range",
                    ...
                    "description": (
                        "The two Treasury-curve spread endpoints carry no probability or coverage "
                        "claim."
                    ),
                },
            ],
            "deliberate_failure_modes": [
                {
                    ...
                    "description": (
                        "TrialCourt must reject the retrospective two-date attempt rather than "
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced Treasury-curve boundary because four "
                "pre-decision native-vintage yield facts, conservative date-plus-two-day knowledge "
                "timing, a separately locked post-decision DGS2/DGS10 pair, an immutable input "
                "lock, separated truth labels, an explicit latest-spread persistence baseline, a "
                "no-probability bounded spread program, deliberate TrialCourt failure, "
                "deterministic outputs, limitations, and a clean-worktree double rebuild all "
                "verify. The March 15 pair is excluded from ReplayPack inputs and used only for a "
                "labelled post-event check that preserves its 6-basis-point range breach. This "
                "does not establish forecast skill, calibrated coverage, stationary behavior, "
                "causal banking, recession, or policy attribution, external validation, "
