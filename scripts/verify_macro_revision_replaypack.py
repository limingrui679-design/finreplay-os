#!/usr/bin/env python3
"""Fresh-run relevant engines and byte-compare a macro revision ReplayPack."""
...
def main() -> None:
    ...
    lock = load_macro_revision_input_lock(args.input_lock)
    ...
    with tempfile.TemporaryDirectory(prefix="finreplay-macro-revision-") as temporary:
        temporary_root = Path(temporary)
        first_spec = build_macro_revision_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
        first_root = studio.build(first_spec, temporary_root / "first").root
        second_spec = build_macro_revision_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
    ...
    event_value = Decimal(str(event_lock.records[0].payload["value"]))
    q4_advance = Decimal(str(shock["naive_baseline"]["q4_gdp_billions"]))
    realized_revision = event_value - q4_advance
    lower = Decimal(str(shock["bound_construction"]["lower_revision_billions"]))
    upper = Decimal(str(shock["bound_construction"]["upper_revision_billions"]))
    assertions = {
        ...
        "naive_baseline_is_zero_revision": (
            float(shock["naive_baseline"]["revision_billions"]) == 0.0
        ),
        ...
        "reported_post_event_revision_inside_declared_envelope": lower
        <= realized_revision
        <= upper,
    ...
    if not all(assertions.values()):
        ...
        raise SystemExit(f"macro revision assertions failed: {failed}")
    semantic: dict[str, Any] = {
        ...
        "claim_boundary": (
            ...
            "ALFRED facts. It also verifies that the separately locked post-decision estimate is "
            "disjoint and, as an evaluation only, falls inside the previously declared no-"
            "probability envelope. It does not prove forecasting skill, calibrated coverage, "
            "causal correctness, deployment, policy value, external review, or user impact."
        ),
        "assertions": assertions,
        "input_lock_sha256": lock.lock_sha256,
        "event_lock_sha256": event_lock.lock_sha256,
        "post_event_evaluation": {
            "reported_q4_second_estimate_billions": format(event_value, "f"),
            "reported_revision_from_advance_billions": format(realized_revision, "f"),
            "declared_lower_revision_billions": format(lower, "f"),
            "declared_upper_revision_billions": format(upper, "f"),
    ...
    if args.write_receipt:
        ...
        raise SystemExit("macro revision receipt is missing; use --write-receipt once")
    print(
...
def _verify_stored_receipt(
    ...
    if receipt_sha256 != _hash(stored):
        raise SystemExit("stored macro revision receipt_sha256 mismatch")
    stored.pop("runtime", None)
    semantic_sha256 = stored.pop("semantic_sha256", None)
    if semantic_sha256 != _hash(stored):
        raise SystemExit("stored macro revision semantic_sha256 mismatch")
    if semantic_sha256 != expected_semantic_sha256 or stored != expected_semantic:
        raise SystemExit("stored macro revision semantic receipt differs from fresh rebuild")
