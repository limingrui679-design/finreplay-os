#!/usr/bin/env python3
"""Build and verify the eight-gate proof for the BLS import-price boundary."""
...
REQUIRED_ASSERTIONS = (
    ...
    "post_event_lower_bound_breach_equals_130_basis_points",
...
def main() -> None:
    ...
    build_script = root / "scripts/build_import_price_boundary_replaypack.py"
    verify_script = root / "scripts/verify_import_price_boundary_replaypack.py"

    lock = load_import_price_boundary_input_lock(input_path)
    ...
    if (
        ...
        raise SystemExit("event-lock identity does not match import-price input lock")
    ...
    if (compiled.spec.scenario_id, compiled.spec.scenario_version, compiled.spec.replay_id) != (
        ...
        raise SystemExit("ReplayPack identity does not match import-price input lock")
    ...
    range_artifact = f"{prefix}.shockcompiler.all-imports-change-range"
    ...
    proof = seal_scenario_proof(
        {
            ...
            "official_adapter_inventory": (
                "verification/supporting/bls-import-prices/latest-summary.json"
            ),
            "input_locks": [
                {
            ...
            "input_labels": {
                ...
                "absence_reasons": {
                    "observed": (
                        "The lock contains aggregate all-import price changes reported in "
                        "archived BLS releases, not observations of importer firms, shipments, "
                        "quantities, nominal trade values, tariffs, CPI, P&L, decisions, or user "
                        "activity. The March change exists only in the disjoint event lock."
                    )
                },
            },
            "naive_baselines": [
                {
                    ...
                    "payload_pointer": (
                        "/naive_baseline/next_all_imports_monthly_change_basis_points"
                    ),
                    "expected_value": -50,
                    "description": (
                        "The decision-time baseline persists the February first-reported "
                        "all-import monthly change of -50 basis points."
                    ),
                },
                {
                    "artifact_id": range_artifact,
                ...
                },
                {
                    ...
                    "expected_value": False,
                    "description": (
                        "The later March all-import change never enters range construction."
                    ),
                },
                {
                    "artifact_id": range_artifact,
            ...
            "claim_boundary": (
                "This proof counts one internally reproduced February 2020 BLS all-import "
                ...
                "truth labels, a -50-basis-point February persistence baseline, a no-probability "
                "-100-to--50-basis-point persistence-or-one-known-decline program, deliberate "
                ...
                "visible but sets no endpoint. The March first report of -230 basis points is "
                "excluded from ReplayPack inputs and used only for a labelled post-event check. "
                "It lies 130 basis points below the fixed lower endpoint; that miss remains "
                "visible, does not become forecast success, and does not alter the endpoints. "
                "Index levels, annual changes, detailed categories, and later revisions remain "
                "metadata. The modified-Laspeyres, U.S.-importer-price, non-seasonally-adjusted, "
                "three-release revision, and COVID-methodology boundaries are preserved. This "
                "proof does not establish a BLS forecast, calibrated coverage, import quantity "
                "or nominal trade value, tariff or CPI effect, importer or firm behavior, P&L, "
                "pandemic causality, investment performance, external validation, deployment, "
                "or user impact."
            ),
        }
    )
