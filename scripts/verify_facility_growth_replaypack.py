#!/usr/bin/env python3
"""Fresh-run relevant engines and byte-compare a BTFP growth ReplayPack."""
...
def main() -> None:
    ...
    lock = load_facility_growth_input_lock(args.input_lock)
    ...
    with tempfile.TemporaryDirectory(prefix="finreplay-facility-growth-") as temporary:
        temporary_root = Path(temporary)
        first_spec = build_facility_growth_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
        first_root = studio.build(first_spec, temporary_root / "first").root
        second_spec = build_facility_growth_replay_spec(
            lock,
            code_commit=compiled.spec.code_commit,
        )
    ...
    event_balance = int(event_lock.records[0].payload["value_millions"])
    prior_balance = int(shock["naive_baseline"]["next_wednesday_balance_millions"])
    realized_growth = event_balance - prior_balance
    lower = int(shock["bound_construction"]["lower_growth_millions"])
    upper = int(shock["bound_construction"]["upper_growth_millions"])
    assertions = {
        ...
        "naive_baseline_is_zero_growth": (
            float(shock["naive_baseline"]["next_week_growth_millions"]) == 0.0
        ),
        ...
        "reported_post_event_growth_inside_declared_envelope": lower
        <= realized_growth
        <= upper,
    ...
    if not all(assertions.values()):
        ...
        raise SystemExit(f"facility growth assertions failed: {failed}")
    semantic: dict[str, Any] = {
        ...
        "claim_boundary": (
            ...
            "H.4.1 facts. It verifies the separately locked March 30 balance is disjoint and, as "
            "an evaluation only, its reported growth falls inside the previously declared no-"
            "probability envelope. It does not prove forecast skill, calibrated coverage, causal "
            "systemic stress, policy effectiveness, deployment, external review, or user impact."
        ),
        "assertions": assertions,
        "input_lock_sha256": lock.lock_sha256,
        "event_lock_sha256": event_lock.lock_sha256,
        "post_event_evaluation": {
            "reported_march_30_wednesday_balance_millions": event_balance,
            "reported_growth_from_prior_wednesday_millions": realized_growth,
            "declared_lower_growth_millions": lower,
            "declared_upper_growth_millions": upper,
    ...
    if args.write_receipt:
        ...
        raise SystemExit("facility growth receipt is missing; use --write-receipt once")
    print(
...
def _verify_stored_receipt(
    ...
    if receipt_sha256 != _hash(stored):
        raise SystemExit("stored facility growth receipt_sha256 mismatch")
    stored.pop("runtime", None)
    semantic_sha256 = stored.pop("semantic_sha256", None)
    if semantic_sha256 != _hash(stored):
        raise SystemExit("stored facility growth semantic_sha256 mismatch")
    if semantic_sha256 != expected_semantic_sha256 or stored != expected_semantic:
        raise SystemExit("stored facility growth semantic receipt differs from fresh rebuild")
