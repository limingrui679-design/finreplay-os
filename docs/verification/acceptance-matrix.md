...
|---|---|---|---|
| A1 | TimeVault supports bitemporal append, revision history, and as-of knowledge queries | Unit/property tests plus a real-source golden replay proving future revisions are excluded | PROVEN — atomic/idempotent append, immutable source identity, revision history, historical-safe latest-only filtering and as-of tests pass; a committed real ALFRED GDP replay appends four pre-decision vintages plus the later official revision and proves the latter is absent at the 2023-02-01 decision cutoff but present after availability |
| A2 | TrialCourt records preregistration, every attempt, leakage/multiplicity/regime/execution attacks, and disposition | Immutable trial ledger, method tests against published examples, negative-result fixture | PROVEN — the hash-chain ledger retains contiguous positive/negative attempts and detects tampering; all six attacks, fail-closed dispositions and the rejected retrospective SVB attempt are tested; `trialcourt-holm-method.json` independently expands Holm's 1979 step-down procedure (DOI 10.2307/4615733), binds the implementation hash and exactly matches TrialCourt output. This is internal method comparison, not external review |
| A3 | MarketTwin stores evidence-graded temporal institution/security graphs and bounded contagion | Graph contract tests plus at least one live official multi-source graph | PROVEN — append-only temporal graph tests cover version fallback, latest-only exclusion, exact-versus-bounded edge contracts, monotone propagation, cycles and mutation rejection; the self-hashed SVB receipt binds a historical-safe SEC graph, a current SEC/FDIC/Treasury graph, and a converged bounded stress result. This is public-data construction evidence, not causal or external validation |
| A4 | ShockCompiler distinguishes observed reconstruction, bounds, counterfactual, and adversarial perturbations | Scenario compiler tests and machine-readable provenance for every shock | PROVEN — mode-specific contracts reject label/source mismatches and grid explosion; the self-hashed SVB receipt validates reported exact shocks, a sourced inferred interval, one source-free simulated counterfactual and a 12-vector source-free simulated adversarial grid, preserving record IDs, content hashes, derivations and limitations through compilation and the seven-engine ReplayPack |
...
|---|---|---|---|
| D1 | At least 1,000,000,000 distinct public-source records actually processed | Source-partition manifests with independent counts, hashes, no synthetic multiplication, and no double counting | PROVEN — the self-hashed manifest seals 244 continuous official SEC daily ZIP coordinates, unique ZIP/receipt/coordinate hashes, and exactly 1,014,736,394 physical CSV rows with no estimate, sampling weight, multiplier or semantic deduplication; `target_met=true` |
| D2 | Billion-row point-in-time query demonstrated | Re-runnable benchmark receipt with hardware, cold/warm state, SQL, elapsed time, peak RSS and scanned bytes | PROVEN — two distinct fresh processes each hash-verified all 12,277,974,518 Parquet bytes and scanned 1,014,736,394 rows under separate event/knowledge cutoffs in 55.878863 and 55.944395 seconds; the self-hashed receipt records SQL, DuckDB/hardware, input/result hashes, peak RSS and explicitly uncontrolled OS cache |
| D3 | Scalable local/object-store layout | Partition-pruning and incremental-ingestion tests; interrupted-run recovery | PROVEN — daily raw/Parquet partitions, atomic writes, bounded four-worker ingestion, ETag/Last-Modified range resume, an actually resumed download, incremental checkpoints, 244-partition pruning/eligibility and a successful full query are committed; this proves the local layout, not a hosted object-store deployment |
| D4 | Scale claim is reproducible without shipping restricted raw data | Downloaders, source locks, sampled fixtures and manifest verifier | PROVEN — two official annual inventory locks, accountable resumable downloader, hostile ZIP/CSV fixtures, 244 download/partition receipts, a manifest rebuilder, and a 1,404.358697-second fresh all-byte deep pass are committed; the small evidence chain is clean-checkout verifiable while an independent byte-level rerun must re-download the ignored official archives |
...
`verification/evidence/internal-quality-gates.json` receipt binds a clean `80bca81a1816` subject
...
non-disclosing local scanner checked 1,441 tracked text files and 14,338,654 UTF-8 bytes with zero
