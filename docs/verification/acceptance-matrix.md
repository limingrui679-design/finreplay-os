...
Current status: **IN_PROGRESS; 11/30 live-validated adapters.** Eight FDIC BankFind
products and three SEC EDGAR/XBRL products have current live receipts under
`verification/live/`. Of these, the FDIC current-index products deliberately remain
`latest_only`; the SEC accession/event products are historical-replay eligible. A count here
proves connector behavior and live retrieval, not that every adapter supplies historical vintages.
