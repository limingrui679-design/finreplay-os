...
| A8 | All seven run in one deterministic end-to-end flow | Fresh-clone rebuild with cross-engine trace IDs and byte/semantic comparison | IN_PROGRESS — locked seven-record SEC input and actual six-analysis-engine plus ReplayStudio flow pass locally; clean-checkout committed pack and rebuild receipt are being finalized |
