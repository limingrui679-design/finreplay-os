# BLS Import Price Index supporting evidence

This directory contains the committed live receipts used by the March 2020 all-import-price
...
identity, headline all-import monthly change, prior-month revision lineage, Table 1 values, the
modified-Laspeyres and non-seasonal-adjustment technical note, the three-month revision policy,
and the March COVID-19 survey-methodology statement. Later revisions remain in later snapshots and
never replace the first-reported January or February decision inputs.
...
```bash
PYTHONPATH=src .venv/bin/python scripts/validate_bls_import_prices.py
PYTHONPATH=src .venv/bin/python scripts/validate_bls_import_prices.py
PYTHONPATH=src .venv/bin/python scripts/verify_live_receipts.py \
  --directory verification/supporting/bls-import-prices/live \
  --raw-store data/raw/supporting/bls-import-prices \
  --output verification/supporting/bls-import-prices/latest-summary.json
```
