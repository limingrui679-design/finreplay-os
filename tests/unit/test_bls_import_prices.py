...
from finreplay.adapters import BLSImportPriceArchiveAdapter, SourceSchemaError
...
RELEASES = {
    date(2020, 2, 14): {
        ...
        "table_row": "All commodities 100.000 125.0 125.0 0.3 -0.4 0.2 0.2 0.0",
        "value": 0,
        "prior_value": 2,
        "previous_value": None,
        "revision_delta": None,
    ...
    date(2020, 3, 13): {
        ...
        "table_row": "All commodities 100.000 125.0 124.4 -1.2 0.2 0.2 0.1 -0.5",
        "value": -5,
        "prior_value": 1,
        "previous_value": 0,
        "revision_delta": 1,
    ...
    date(2020, 4, 14): {
        ...
        "table_row": "All commodities 100.000 124.3 121.4 -4.1 0.2 0.2 -0.7 -2.3",
        "value": -23,
        "prior_value": -7,
        "previous_value": -5,
        "revision_delta": -2,
...
def pdf_bytes(
    *,
    ...
    if pages > 4:
        page_lines[4] = [
            "Table 1. U.S. import price indexes and percent changes",
            str(spec["table_row"]),
        ]
    if pages > 15:
...
def adapter(
    *,
    ...
    return BLSImportPriceArchiveAdapter(safe, release_date=release_date)


def test_import_price_pair_is_exact_versioned_and_knowledge_safe() -> None:
    html = html_bytes()
    pdf = pdf_bytes()
    ...
    assert record.record_id.endswith("202002:all_imports_monthly_change")
    assert record.entity_id == "bls_import_price_index:all_imports_united_states"
    ...
    assert record.payload["value_tenths_percent"] == -5
    assert record.payload["value_basis_points"] == -50
    assert record.payload["prior_month_change_tenths_percent"] == 1
    assert record.payload["prior_month_revision_delta_tenths_percent"] == 1
    assert record.payload["table1_current_unadjusted_index"] == "124.4"
...
def test_verified_calendar_preserves_first_reports_revisions_and_timing(
    release_date: date,
) -> None:
    spec = RELEASES[release_date]
...
def test_verified_calendar_and_response_urls_are_closed() -> None:
    ...
    with pytest.raises(ValueError, match="verified BLS import-price calendar"):
        BLSImportPriceArchiveAdapter(client, release_date=date(2020, 5, 14))

    item = BLSImportPriceArchiveAdapter(client, release_date=RELEASE_DATE)
    for kind, invalid in (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (pdf_bytes(replacements={"Table 1.": "Other Table."}), "Table 1 page"),
...
def test_pdf_structure_and_identity_fail_closed(pdf: bytes, message: str) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"declined 0.5 percent": "rose 0.5 percent"}, "identity or headline"),
        ({"124.4 -1.2": "124.3 -1.2"}, "Table 1 values"),
...
def test_html_corruption_fails_closed(
...
def test_covid_marker_is_required_only_for_march_release() -> None:
    ...
    with pytest.raises(SourceSchemaError, match="COVID-19 methodology"):
        adapter(
            html=html_bytes() + f"<p>{COVID_MARKER}</p>".encode(),
        ).fetch()
...
def test_response_url_and_prepublication_retrieval_fail_closed() -> None:
    ...
    wrong = BLSImportPriceArchiveAdapter(
    ...
    early = BLSImportPriceArchiveAdapter(
