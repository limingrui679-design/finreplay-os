...
LOCK_PATH = Path("scenarios/fed-g17-2020/input-lock.json")
EVENT_PATH = Path("scenarios/fed-g17-2020/event-lock.json")
PACK_PATH = Path("verification/replaypacks/fed-g17-2020")
CODE_COMMIT = "fe4a64544e22accedac1947dd8f772b538812a0f"


@pytest.mark.integration
def test_industrial_production_boundary_runs_four_engines_with_exact_values() -> None:
    lock = load_industrial_production_boundary_input_lock(LOCK_PATH)
    spec = build_industrial_production_boundary_replay_spec(lock, code_commit=CODE_COMMIT)
    ...
    assert shock.payload["known_changes"] == {
        ...
        "february_monthly_change_basis_points": 60,
        "known_increase_basis_points": 90,
        "lower_change_basis_points": 60,
        "upper_change_basis_points": 150,
        "range_width_basis_points": 90,
    }
    assert shock.payload["naive_baseline"] == {
        "next_monthly_change_basis_points": 60,
...
def test_march_event_is_later_disjoint_revised_and_breaches_lower_bound() -> None:
    ...
    assert record.payload["release_series"] == "G.17 (419)"
    assert record.payload["value_basis_points"] == -540
    assert record.payload["reported_monthly_change_percent"] == "-5.4"
    assert record.payload["total_index_2012_equals_100"] == "103.7"
    assert record.payload["capacity_utilization_percent"] == "72.7"
    assert record.payload["prior_month_change_in_current_release_basis_points"] == 50
    assert record.payload["prior_month_change_in_previous_release_basis_points"] == 60
    ...
    assert 60 - record.payload["value_basis_points"] == 600
    february = next(
    ...
    assert february.payload["prior_month_revision_delta_basis_points"] == -20
...
@pytest.mark.parametrize(
    ("case", "message"),
    [
        ("wrong_source", "only archived industrial-production facts"),
        ...
        ("total_index_2012_equals_100", "total_index_2012_equals_100 mismatch"),
        ("capacity_utilization_percent", "capacity_utilization_percent mismatch"),
        ("manufacturing_monthly_change_percent", "manufacturing_monthly_change_percent mismatch"),
        ("mining_monthly_change_percent", "mining_monthly_change_percent mismatch"),
        ("utilities_monthly_change_percent", "utilities_monthly_change_percent mismatch"),
        ("year_over_year_change_percent", "year_over_year_change_percent mismatch"),
        (
            "prior_month_change_in_current_release_basis_points",
            "prior_month_change_in_current_release_basis_points mismatch",
        ),
        (
            "prior_month_change_in_previous_release_basis_points",
            "prior_month_change_in_previous_release_basis_points mismatch",
        ),
        (
        ...
        ("summary_table_snapshot_verified", "summary_table_snapshot_verified mismatch"),
        ("release_html_url", "release_html_url mismatch"),
        ("release_html_fact_sha256", "release_html_fact_sha256 mismatch"),
...
def test_industrial_production_lock_rejects_source_timing_and_value_corruption(
    case: str,
    message: str,
) -> None:
    ...
    if case == "wrong_source":
        ...
        first["entity_id"] = "federal_reserve_g17:other"
    elif case == "low_confidence":
        first["interval"]["availability_confidence"] = 0.5
    elif case == "wrong_published":
        first["interval"]["published_at"] = "2020-02-14T14:14:59Z"
    elif case == "wrong_available":
        first["interval"]["available_at"] = "2020-02-14T14:15:01Z"
    elif case == "future":
        values["decision_time"] = "2020-02-14T14:14:59Z"
    elif case == "wrong_valid":
        first["interval"]["valid_from"] = "2020-01-02T00:00:00Z"
    elif case == "wrong_vintage":
        first["source"]["vintage_as_of"] = "2020-02-14T14:15:01Z"
        ...
        first["source"]["url"] = "https://www.federalreserve.gov/releases/g17/other.pdf"
    elif case == "wrong_source_version":
        first["source"]["source_version"] = "FED-G17:fabricated"
    else:
        first["payload"][case] = _corrupt(first["payload"][case])
    with pytest.raises(ValidationError, match=message):
        IndustrialProductionBoundaryInputLock.model_validate(values)
...
def test_industrial_production_lock_rejects_manifest_role_and_clock_corruption(
    case: str,
    message: str,
) -> None:
    values = json.loads(LOCK_PATH.read_text(encoding="utf-8"))
    if case == "early_build":
        ...
        values["roles"]["january_monthly_change"] = values["roles"]["february_monthly_change"]
        ...
        values["build_epoch"] = "2026-08-13T09:10:00"
    with pytest.raises(ValidationError, match=message):
        IndustrialProductionBoundaryInputLock.model_validate(values)


@pytest.mark.integration
def test_industrial_production_lock_creation_loading_hash_and_runtime_guards(
    tmp_path: Path,
) -> None:
    values = json.loads(LOCK_PATH.read_text(encoding="utf-8"))
    values.pop("lock_sha256")
    recreated = IndustrialProductionBoundaryInputLock.create(values)
    assert recreated == IndustrialProductionBoundaryInputLock.model_validate_json(
        LOCK_PATH.read_text(encoding="utf-8")
    )

    invalid = tmp_path / "invalid-lock.json"
    invalid.write_text("{}", encoding="utf-8")
    with pytest.raises(ValueError, match=r"invalid G\.17 industrial-production input lock"):
        load_industrial_production_boundary_input_lock(invalid)
    with pytest.raises(ValueError, match=r"invalid G\.17 industrial-production input lock"):
        load_industrial_production_boundary_input_lock(tmp_path / "missing-lock.json")

    tampered = json.loads(LOCK_PATH.read_text(encoding="utf-8"))
    tampered["title"] = "Fabricated Federal Reserve G.17 production boundary title"
    with pytest.raises(ValidationError, match="lock_sha256"):
        IndustrialProductionBoundaryInputLock.model_validate(tampered)
    ...
    with pytest.raises(ValueError, match="must establish a positive monthly-change step"):
        build_industrial_production_boundary_replay_spec(bypassed, code_commit=CODE_COMMIT)


@pytest.mark.integration
@pytest.mark.parametrize("invalid_value", [True, -30.5, -100_001, 100_001])
def test_industrial_production_replay_fails_closed_for_invalid_bypassed_change(
