...
LOCK_PATH = Path("scenarios/bls-cpi-2023/input-lock.json")
EVENT_PATH = Path("scenarios/bls-cpi-2023/event-lock.json")
CODE_COMMIT = "e" * 40


@pytest.fixture(scope="module")
def lock() -> CPIBoundaryInputLock:
    return CPIBoundaryInputLock.model_validate_json(
...
@pytest.fixture(scope="module")
def spec(lock: CPIBoundaryInputLock) -> ReplayPackSpec:
    return build_cpi_boundary_replay_spec(lock, code_commit=CODE_COMMIT)
...
def test_bls_inputs_and_post_decision_event_are_exact_and_disjoint(
    ...
    assert by_id[lock.roles.december_monthly].payload["value_tenths_percent"] == -1
    assert by_id[lock.roles.december_yoy].payload["value_tenths_percent"] == 65
    assert by_id[lock.roles.january_monthly].payload["value_tenths_percent"] == 5
    assert by_id[lock.roles.january_yoy].payload["value_tenths_percent"] == 64
    assert event_lock.records[0].payload["value_tenths_percent"] == 4
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("negative_yoy", "12-month change is outside"),
    ],
)
def test_cpi_lock_rejects_temporal_source_and_value_inflation(
    case: str,
    message: str,
) -> None:
    ...
    if case == "future":
        future = (
            CPIBoundaryInputLock.model_validate(values).decision_time
            + timedelta(seconds=1)
        ).isoformat()
    ...
    elif case == "negative_yoy":
        first["payload"]["value_tenths_percent"] = -1
    with pytest.raises(ValidationError, match=message):
        CPIBoundaryInputLock.model_validate(values)


@pytest.mark.integration
def test_cpi_lock_rejects_role_and_self_hash_tamper() -> None:
    values = json.loads(LOCK_PATH.read_text(encoding="utf-8"))
    values["roles"]["december_yoy"] = values["roles"]["december_monthly"]
    with pytest.raises(ValidationError, match="must be unique"):
        CPIBoundaryInputLock.model_validate(values)

    values = json.loads(LOCK_PATH.read_text(encoding="utf-8"))
    values["title"] = "Fabricated BLS CPI monthly change release boundary title"
    with pytest.raises(ValidationError, match="lock_sha256"):
        CPIBoundaryInputLock.model_validate(values)


@pytest.mark.integration
def test_cpi_boundary_runs_relevant_engines_and_preserves_boundaries(
    spec: ReplayPackSpec,
) -> None:
...
@pytest.mark.integration
def test_cpi_boundary_pack_builds_byte_identically(
    spec: ReplayPackSpec,
    tmp_path: Path,
) -> None:
    studio = ReplayStudio()
    ...
    assert compiled.topological_artifact_ids[0] == "bls-cpi.timevault.release-query"
    assert compiled.topological_artifact_ids[-1] == "bls-cpi.replaystudio.render"
