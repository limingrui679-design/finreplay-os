...
CENSUS_C30_PROOF_PATH = PROOF_DIRECTORY / "census-c30-2020-construction-spending-boundary-v1.json"
...
def test_committed_proofs_and_deterministic_catalog_are_fully_verified() -> None:
    ...
    btfp_growth = verify_scenario_proof(
        BTFP_GROWTH_PROOF_PATH,
        repository_root=REPOSITORY,
    )
    assert btfp_growth.scenario_id == "btfp-2023-early-growth-boundary"
    ...
    assert fomc_target.scenario_id == "fomc-2023-target-range-boundary"
    ...
    nyfed_sofr = verify_scenario_proof(
        NYFED_SOFR_PROOF_PATH,
        repository_root=REPOSITORY,
    )
    assert nyfed_sofr.scenario_id == "nyfed-sofr-2019-spike-boundary"
    assert nyfed_sofr.mode == "bounded_reconstruction"
    assert nyfed_sofr.distinct_input_records == 2

    eia_wpsr = verify_scenario_proof(
        EIA_WPSR_PROOF_PATH,
        repository_root=REPOSITORY,
    )
    assert eia_wpsr.scenario_id == "eia-wpsr-2020-crude-stock-boundary"
    ...
    bea_pio = verify_scenario_proof(
        BEA_PIO_PROOF_PATH,
        repository_root=REPOSITORY,
    )
    assert bea_pio.scenario_id == "bea-pio-2020-saving-rate-boundary"
    ...
    census_nrc = verify_scenario_proof(
        CENSUS_NRC_PROOF_PATH,
        repository_root=REPOSITORY,
    )
    assert census_nrc.scenario_id == "census-nrc-2020-housing-starts-boundary"
    assert census_nrc.mode == "bounded_reconstruction"
    assert census_nrc.distinct_input_records == 2

    fed_g19 = verify_scenario_proof(
        FED_G19_PROOF_PATH,
        repository_root=REPOSITORY,
    )
    assert fed_g19.scenario_id == "fed-g19-2020-revolving-credit-boundary"
    ...
    assert census_c30.scenario_id == "census-c30-2020-construction-spending-boundary"
    assert census_c30.mode == "bounded_reconstruction"
    assert census_c30.distinct_input_records == 2

    fhfa_hpi = verify_scenario_proof(
        FHFA_HPI_PROOF_PATH,
        repository_root=REPOSITORY,
    )
    assert fhfa_hpi.scenario_id == "fhfa-hpi-2020-house-price-change-boundary"
    assert fhfa_hpi.mode == "bounded_reconstruction"
    assert fhfa_hpi.distinct_input_records == 2

    census_m3 = verify_scenario_proof(
        CENSUS_M3_PROOF_PATH,
        repository_root=REPOSITORY,
    )
    assert census_m3.scenario_id == "census-m3-2020-durable-goods-change-boundary"
    assert census_m3.mode == "bounded_reconstruction"
    assert census_m3.distinct_input_records == 2

    census_ft900 = verify_scenario_proof(
        CENSUS_FT900_PROOF_PATH,
        repository_root=REPOSITORY,
    )
    assert census_ft900.scenario_id == "census-ft900-2020-trade-deficit-level-boundary"
    assert census_ft900.mode == "bounded_reconstruction"
    assert census_ft900.distinct_input_records == 2

    census_nrs = verify_scenario_proof(
        CENSUS_NRS_PROOF_PATH,
        repository_root=REPOSITORY,
    )
    assert census_nrs.scenario_id == "census-nrs-2020-new-home-sales-level-boundary"
    ...
    cftc_tff = verify_scenario_proof(
        CFTC_TFF_PROOF_PATH,
        repository_root=REPOSITORY,
    )
    assert cftc_tff.scenario_id == "cftc-tff-2026-two-year-note-open-interest-boundary"
    ...
    bls_import_price = verify_scenario_proof(
        BLS_IMPORT_PRICE_PROOF_PATH,
        repository_root=REPOSITORY,
    )
    assert (
