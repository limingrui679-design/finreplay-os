...
LOCK_PATH = Path("scenarios/bls-import-prices-2020/input-lock.json")
EVENT_PATH = Path("scenarios/bls-import-prices-2020/event-lock.json")
CODE_COMMIT = "0" * 40


@pytest.mark.integration
def test_import_price_boundary_runs_four_engines_from_archived_release_pairs() -> None:
    lock = load_import_price_boundary_input_lock(LOCK_PATH)
    spec = build_import_price_boundary_replay_spec(lock, code_commit=CODE_COMMIT)
    ...
    assert timevault.payload["decision_observations_basis_points"] == {
        "january_all_imports_monthly_change": 0,
        "february_all_imports_monthly_change": -50,
        "february_prior_january_change": 10,
    ...
    assert shock.payload["known_release_changes"] == {
        "january_change_basis_points": 0,
        "february_change_basis_points": -50,
        "known_decline_basis_points": 50,
        "lower_change_basis_points": -100,
        "upper_change_basis_points": -50,
        "range_width_basis_points": 50,
    }
    ...
    assert shock.payload["applied_endpoints"] == {"lower": -100.0, "upper": -50.0}
    parameter = shock.payload["program"]["parameters"][0]
    assert parameter["unit"] == "basis_points"
    assert parameter["lower"] == -100.0
    assert parameter["upper"] == -50.0
    ...
    assert january.payload["value_basis_points"] == 0
    ...
    assert february.payload["prior_month_change_tenths_percent"] == 1
    assert february.payload["prior_month_value_in_previous_release_tenths_percent"] == 0
...
@pytest.mark.integration
def test_march_event_is_disjoint_and_misses_the_fixed_lower_endpoint() -> None:
    lock = load_import_price_boundary_input_lock(LOCK_PATH)
    ...
    assert record.payload["value_tenths_percent"] == -23
    assert record.payload["value_basis_points"] == -230
    assert record.payload["prior_month_change_tenths_percent"] == -7
    assert record.payload["prior_month_value_in_previous_release_tenths_percent"] == -5
    assert record.payload["prior_month_revision_delta_tenths_percent"] == -2
    ...
    assert -100 - record.payload["value_basis_points"] == 130

    spec = build_import_price_boundary_replay_spec(lock, code_commit=CODE_COMMIT)
    shock = next(
        artifact for artifact in spec.artifacts if artifact.engine is EngineName.SHOCKCOMPILER
    )
    assert shock.payload["bound_construction"]["lower_change_basis_points"] == -100
    assert shock.payload["bound_construction"]["upper_change_basis_points"] == -50
...
def test_import_price_lock_rejects_source_timing_and_payload_corruption(
    case: str,
    message: str,
) -> None:
    values = _lock_values()
    ...
    if case == "wrong_source":
        first["source"]["source_id"] = "bls.import_prices.other"
        ...
        first["entity_id"] = "bls_import_price_index:other"
        ...
        first["source"]["retrieved_at"] = "2026-08-14T11:15:01Z"
        first["interval"]["ingested_at"] = "2026-08-14T11:15:01Z"
    ...
    with pytest.raises((ValidationError, ValueError), match=message):
        ImportPriceBoundaryInputLock.model_validate(values)


@pytest.mark.integration
def test_import_price_lock_rejects_hash_roles_decision_and_self_hash_corruption() -> None:
    ...
    with pytest.raises(ValidationError, match="four official responses"):
        ImportPriceBoundaryInputLock.model_validate(wrong_hashes)
    ...
    with pytest.raises(ValidationError, match="supporting receipt"):
        ImportPriceBoundaryInputLock.model_validate(wrong_receipt)
    ...
    with pytest.raises(ValidationError, match="role record IDs must be unique"):
        ImportPriceBoundaryInputLock.model_validate(duplicate_roles)
    ...
    with pytest.raises(ValidationError, match="March 13 embargo end"):
        ImportPriceBoundaryInputLock.model_validate(wrong_decision)

    wrong_self_hash = _lock_values()
    wrong_self_hash["lock_sha256"] = "0" * 64
    with pytest.raises(ValidationError, match="lock_sha256"):
        ImportPriceBoundaryInputLock.model_validate(wrong_self_hash)


@pytest.mark.integration
def test_import_price_event_lock_rejects_inexact_timing() -> None:
